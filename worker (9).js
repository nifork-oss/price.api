 ...
 *
 * ФОТО/ФАЙЛЫ ОТЧЁТОВ хранятся в Cloudflare R2 (не в JSONBin — там только
 * текст). Разовая настройка перед деплоем:
 *   1. Cloudflare Dashboard -> R2 -> Create bucket, назовите его,
 *      например, "prise-reports".
 *   2. В настройках вашего Worker (Settings -> Bindings -> Add binding ->
 *      R2 Bucket) добавьте привязку с именем переменной REPORTS_BUCKET,
 *      указав созданный бакет.
 *   Если бакет не привязан, загрузка файлов будет возвращать ошибку, но
 *      остальной сайт продолжит работать как обычно.
 *
...
export default {
  async fetch(request, env) {
    ...
    try {
      ...
      // Выдача файла отчёта из R2. Специально БЕЗ проверки входа — иначе
      // фото нельзя было бы вставить обычным <img src="..."> или дать
      // заказчику прямую ссылку без лишних сложностей на фронтенде.
      // Ключ файла — случайный (см. /upload) и не угадывается, поэтому
      // это приемлемый компромисс для данного сайта.
      if (path === "/file" && request.method === "GET") {
        if (!env.REPORTS_BUCKET) return json({ error: "Хранилище файлов не подключено" }, 500);
        const key = url.searchParams.get("key") || "";
        if (!key.startsWith("reports/")) return json({ error: "Некорректный файл" }, 400);
        const obj = await env.REPORTS_BUCKET.get(key);
        if (!obj) return json({ error: "Файл не найден" }, 404);
      ...
      // Загрузка файла отчёта (фото/документ) в R2. Заказчику — только
      // просмотр, загружать и удалять файлы может мастер или админ.
      if (path === "/upload" && request.method === "POST") {
        if (auth.role === "client") return json({ error: "Недостаточно прав" }, 403);
        if (!env.REPORTS_BUCKET) return json({ error: "Хранилище файлов не подключено. Настройте R2-бакет в Cloudflare (см. комментарий в начале worker.js)." }, 500);
        ...
        const ext = safeName.includes(".") ? safeName.split(".").pop().replace(/[^a-zA-Z0-9]/g, "").slice(0, 10) : "";
        const key = `reports/${Date.now()}_${crypto.randomUUID()}${ext ? "." + ext : ""}`;
        await env.REPORTS_BUCKET.put(key, file.stream(), {
          httpMetadata: { contentType: file.type || "application/octet-stream" },
        });
        return json({ key, name: safeName, size: file.size, type: file.type || "" });
      }

      // Удаление файла отчёта из R2.
