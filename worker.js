  ...
  async fetch(request, env) {
    ...
    try {
      ...
      if (path === "/public-company" && request.method === "GET") {
        ...
        return json({ services: cd.services || [], companyName: user.companyName || companyLogin });
      }
      ...
      if (path === "/appdata" && request.method === "GET") {
        ...
        const self = selfUser
          ? { login: selfUser.login, email: selfUser.email || "", companyName: selfUser.companyName || "", publicPriceEnabled: !!selfUser.publicPriceEnabled }
          : { login: auth.login, email: "", companyName: "", publicPriceEnabled: false };
      ...
      if (path === "/appdata" && request.method === "PUT") {
        ...
        if (incoming.self && typeof incoming.self === "object") {
          ...
          return json({
            self: { login: selfUser.login, email: selfUser.email || "", companyName: selfUser.companyName || "", publicPriceEnabled: !!selfUser.publicPriceEnabled },
            token: newToken,
          });
        }
