  ...
  async fetch(request, env) {
    ...
    try {
      ...
      if (path === "/appdata" && request.method === "GET") {
        ...
        const self = selfUser
          ? { login: selfUser.login, email: selfUser.email || "", companyName: selfUser.companyName || "" }
          : { login: auth.login, email: "", companyName: "" };
      ...
      if (path === "/appdata" && request.method === "PUT") {
        ...
        if (incoming.self && typeof incoming.self === "object") {
          ...
          return json({
            self: { login: selfUser.login, email: selfUser.email || "", companyName: selfUser.companyName || "" },
            token: newToken,
          });
        }
