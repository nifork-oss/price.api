...
export default {
  async fetch(request, env) {
    ...
    try {
      // ВРЕМЕННАЯ диагностика — проверяет, видит ли worker секреты Cloudinary
      // вообще (без раскрытия самих значений). Уберите этот блок, когда
      // загрузка файлов заработает и надобность в проверке отпадёт.
      if (path === "/debug-env" && request.method === "GET") {
        return json({
          cloudNameSet: !!env.CLOUDINARY_CLOUD_NAME,
          apiKeySet: !!env.CLOUDINARY_API_KEY,
          apiSecretSet: !!env.CLOUDINARY_API_SECRET,
          cloudNamePreview: env.CLOUDINARY_CLOUD_NAME ? env.CLOUDINARY_CLOUD_NAME.slice(0, 3) + "..." : null,
        });
      }
